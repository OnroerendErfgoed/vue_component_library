...
describe('OeZoneerder', () => {
  describe('default', () => {
    ...
    it('activates the single configured option by default without opening the panel', () => {
      cy.mount(TestComponent, {
        props: {
          drawPanelEnabled: true,
          featureSelectConfig: {
      ...
      cy.dataCy('zonePanelControl').should('not.exist');
      cy.dataCy('selectPerceelButton').should('not.exist');
      cy.get('.map').should('have.class', 'featureSelect');
    });

    it('keeps the panel open when WKT is the only configured option', () => {
      cy.mount(TestComponent, {
        props: {
          drawPanelEnabled: true,
          featureSelectConfig: {
      ...
      cy.dataCy('zonePanelControl').should('exist');
      cy.dataCy('WKTInput').should('exist');
      cy.dataCy('plaatsWKT').should('exist');
      cy.get('.panel .pointer').click();
      cy.get('.panel').should('have.class', 'closed');
    });

    it('does not add zones above maxZones limit', () => {
      cy.mount(TestComponent, {
        props: {
          drawPanelEnabled: true,
          maxZones: 1,
    ...
    it('emits up-to-date currentZones when limit is reached', () => {
      ...
      const EventTestComponent = defineComponent({
        components: { OeZoneerder },
        setup() {
          const zoneLimitPayload = ref('none');
          const onZoneLimitReached = (payload: ZoneLimitReachedEventDetail) => {
            zoneLimitPayload.value = `${payload.currentZones}/${payload.maxZones}:${payload.attemptedAction}`;
          };
        ...
        template: `
          <div>
            ...
            />
            <span data-cy="zoneLimitPayload">{{ zoneLimitPayload }}</span>
          </div>
        `,
      });

      cy.mount(EventTestComponent);
      cy.dataCy('WKTInput').type(wktValue, { delay: 0 });
      cy.dataCy('plaatsWKT').click();
      cy.dataCy('zoneLimitPayload').should('have.text', '1/1:wkt');
    });

    it('allows adding again after removing one zone at maxZones limit', () => {
      cy.mount(TestComponent, {
        props: {
          drawPanelEnabled: true,
          maxZones: 1,
    ...
    it('can reset all drawn zones via component API', () => {
      ...
      const ResettableTestComponent = defineComponent({
        ...
        template: `
          <div>
            <oe-zoneerder ref="zoneerderRef" :draw-panel-enabled="drawPanelEnabled" :zone="zone" style="height: 100vh" />
            <button data-cy="resetZones" @click="reset">Reset</button>
          </div>
        `,
      });

      cy.mount(ResettableTestComponent);
      cy.dataCy('zonePanelControl').click();
      cy.get('.geometryObjectList li').should('have.length', 1);
