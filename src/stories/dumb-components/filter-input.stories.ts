...
export const Default: Story = {
  render: () => ({
    ...
    <filter-input v-slot="{ value, setValue, selectedOption }" :options="filterOptions" @filters-selected="$event => filters = $event">
      ...
      <filter-gemeente v-if="selectedOption.key === 'gemeente'" api="https://dev-geo.onroerenderfgoed.be/" :value="value" @update:value="setValue($event, $event.naam)"></filter-gemeente>
      <filter-radio v-if="selectedOption.key === 'beheerscommissie' || selectedOption.key === 'beheersplan_verlopen'" :options="radioOptions" :value="value" @update:value="setValue($event, $event.label)"></filter-radio>
